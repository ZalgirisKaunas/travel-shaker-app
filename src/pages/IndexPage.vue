  ...
  <q-page>
    <div class="q-pt-xl q-mt-xl" v-if="step === 0">
      <Lottie-animation
        ...
        :autoPlay="true"
      />
      <h2 class="q-mt-xl">Hold tight while we are finding the best travel deals for you...</h2>
      <h5 class="feedback text-center">{{ feedback }}</h5>
    </div>

    <form-step v-if="step === 1" @changeStep="changeStepp"
               title="Let’s see where you should travel next!" class="step flex flex-center" :step="step">
      <first-step @changeStep="changeStepp"/>
    </form-step>

    <form-step v-if="step === 2" @changeStep="changeStepp"
               subtitle="It doesn’t have to be that exact place, select similarly looking ones!"
               title="Which places have you visited already?" class="step flex flex-center q-px-lg" :next="false"
               :step="step">
      <second-step v-model="visitedPlaces" @changeStep="changeStepp"/>
    </form-step>

    <form-step v-if="step === 3" @changeStep="changeStepp" subtitle="Let us get to know you a little..."
               title="What is your gender?" class="step flex flex-center" :step="step">
      <gender-select v-model="gender"/>
    </form-step>

    <form-step v-if="step === 4" @changeStep="changeStepp"
               subtitle="We craft a tailor-made trip for you - free of charge."
               title="What is your group size?" class="step flex flex-center" :step="step">
      <q-list>
        <q-item>
    ...
    <form-step v-if="step === 5" @changeStep="changeStepp" subtitle="Weekend getaway of a full long holiday?"
               title="How long would you like
to travel for?" class="step flex flex-center" :step="step">
      <q-list class="radio-group">
        <q-item-section>
          <q-radio left-label v-model="tDuration" val="0" label="Up to 5 days"/>
        </q-item-section>
    ...
    <form-step v-if="step === 6" @changeStep="changeStepp" subtitle="Dream big! We’ll see how we can help."
               title="Which pictures reflect your dream holidays?" class="step flex flex-center" :step="step">
<!--      {{ selectedPhotos }}-->
      <slider-select :board="'1141944117954577838'" v-model="selectedPhotos" />
    </form-step>

    <form-step v-if="step === 7" @changeStep="changeStepp"
               subtitle="Choose up to 3 things that you find most important during your vacations."
               title="What are your top priorities when treavelling?" class="step flex flex-center" :step="step">
      <priorities-select v-model="priorities"/>
    </form-step>

    <form-step v-if="step === 8" @changeStep="changeStepp" subtitle="Let us get to know you a little..."
               title="Which of the experiences are on you bucket list?" class="step flex flex-center" :step="step">
<!--      {{ selectedActivities }}-->
      <slider-select :board="'1141944117954581418'" v-model="selectedActivities" />
    </form-step>
    <form-step v-if="step === 9" @changeStep="changeStepp" subtitle=""
               title="We created a personalised recommendation for you. Where should we send it?"
               class="step flex flex-center overflow-y q-pt-lg" :step="step">
      <div class="what-you-get-screen">
        <email-input v-model="email"  class="q-pb-sm"/>
        <q-btn unelevated flat class="btn-main q-mb-sm"
...
export default defineComponent({
  ...
  setup() {
    ...
    const buildReq = async () => {
      ...
      let visitedCountriesAnalyzed = analyzedPhotos.data
        .map(item => ({
          tags: item.tags,
