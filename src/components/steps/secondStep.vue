<template>
  ...
  >
    <swiper-slide :key="item.image + index" v-for="(item, index) in pinFeedVisited">
      <div
        class="pic"
        :key="index+index"
  ...
  <div class="yes-no-buttons flex">
    <q-btn unelevated flat class="btn-secondary" label="Yes" @click="yesSlide" />
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const onSwiperStack = (swiper) => { swipe.value=swiper };
        const number = ref(props.modelValue);
        const changeStep = (val) => { context.emit('changeStep', val) };

        const getPinterest = async () => {
            api.get('boards/1141944117954581257/pins', {   params: {
                    page_size: 50
                } }).then(async (d) => {
        ...
        return { visitedPhotos, number, changeStep, yesSlide, noSlide, onSwiperStack, pinFeedVisited };
    },
})
</script>
